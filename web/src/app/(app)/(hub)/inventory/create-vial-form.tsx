...
export function CreateVialForm(props: { substances: VialSelectorSubstance[] }) {
  ...
  const [selectedFormulationId, setSelectedFormulationId] = useState(preferredFormulationId)
  ...
  const activeSelection = selectedFormulationId ? formulationToSubstance.get(selectedFormulationId) ?? null : null
  ...
  return (
    <div className="rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark p-4 shadow-sm">
      ...
      <form className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2" action={formAction} data-e2e="vial-create-form">
        <input name="formulation_id" type="hidden" value={selectedFormulationId} required />
        ...
        <div className="sm:col-span-2 rounded-lg border border-border-light dark:border-border-dark bg-slate-50/80 dark:bg-slate-900/30 p-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              {createdFormulationId && createdFormulationId === selectedFormulationId ? (
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      {substance.formulations.map((formulation) => {
                        const selected = formulation.id === selectedFormulationId
                        return (
                          <button
                            key={formulation.id}
                            type="button"
                            ...
                            onClick={() => setSelectedFormulationId(formulation.id)}
