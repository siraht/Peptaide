...
export function ReconcileImportedVialsForm() {
  ...
  return (
    <div className="rounded-lg border bg-white p-4" data-e2e="reconcile-imported-vials">
      <h2 className="text-sm font-semibold text-zinc-900">Spreadsheet migration</h2>
      ...
      {state.status === 'error' ? (
        <p className="mt-3 text-sm text-red-700" data-e2e="reconcile-imported-vials-error">
          {state.message}
        </p>
      ) : null}
      {state.status === 'success' ? (
        <div className="mt-3 space-y-1 text-sm">
          <p className="text-emerald-700" data-e2e="reconcile-imported-vials-success">
            {state.message}
          </p>
