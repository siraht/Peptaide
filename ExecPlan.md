...
- [ ] UI: implement the cycle automation UX in `/today` (gap-based new-cycle suggestion with default-yes behavior that preserves fast entry) and `/cycles` page with split-at-event correction tools and break computation (completed: `/today` server action now assigns cycles automatically; added a minimal `/cycles` summary page; remaining: the prompt UX and correction tools such as split-at-event). plan[261-282] plan[462-540] plan[666-670]
