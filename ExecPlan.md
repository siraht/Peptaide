...
- [x] (2026-02-07 07:45Z) Implemented CSV bundle import v1: added a dry-run validator and an apply mode (`/api/import`) with optional replace mode (delete all user data first, in FK-safe order). Added a "Danger zone" delete endpoint (`/api/delete-my-data`) and a `/settings` UI section for export/import/delete. Import rebases all rows to the current signed-in `user_id` and preserves UUID ids for FK integrity; it currently requires the exact exported header order and is replace-only (no merge/dedupe yet). Files: `web/src/app/api/import/route.ts`, `web/src/app/api/delete-my-data/route.ts`, `web/src/lib/import/csvBundle.ts`, `web/src/lib/import/deleteMyData.ts`, `web/src/lib/import/csv.ts`, `web/src/app/(app)/settings/data-portability.tsx`, `web/src/app/(app)/settings/page.tsx`. plan[583-595] plan[682-689]
