...
- [x] (2026-02-07 04:27Z) Added a minimal `/distributions` page to create/list uncertainty distributions (currently point distributions only) so model specs can reference real distribution rows. Implemented via typed repo helpers (`listDistributions`, `createPointDistribution`) and linked it in the app nav (`web/src/app/(app)/layout.tsx`). plan[304-343] plan[422-461]
