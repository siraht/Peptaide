...
- [ ] UI: implement Setup Wizard A1-A6 (preferences, bulk add substances, bulk add routes, bulk add formulations, bulk add vials and/or generate from orders, base BA + modifiers entry) (completed: a minimal `/setup` onboarding hub exists and embeds profile defaults plus bulk-add primitives for substances/routes/formulations; remaining: add bulk vials (or generate-from-orders) and BA/modifier entry to `/setup`, then refine it into a true wizard flow with completion state). plan[422-461]
