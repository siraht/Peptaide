...
- [x] (2026-02-07 12:28Z) Cycles: implemented gap-based suggestion (`web/src/lib/domain/cycles/suggest.ts` + tests), integrated automatic cycle assignment + the default-yes "New cycle?" prompt into `/today`, and shipped cycle tooling on `/cycles` (list + detail, split-at-event correction, end/abandon, and manual "Start cycle now"). Remaining cycle UX work is tracked in the later `/today` + `/cycles` progress items (richer end-time selection, merge tooling, etc.). plan[261-282] plan[462-540] plan[666-670]
...
- [x] (2026-02-07 14:08Z) Hardening: `public.split_cycle_at_event(...)` now fails fast when the selected event timestamp is earlier than the cycle start timestamp (this edge-case makes "split at this event" ill-defined). Added a stable DB exception string and mapped it to a clear UI error. Files: `supabase/migrations/20260207163000_091_cycle_split_fn_event_before_start.sql`, `web/src/app/(app)/cycles/[cycleInstanceId]/actions.ts`. plan[261-282] plan[462-540]
...
- [x] (2026-02-07 11:46Z) UI: added `/evidence-sources` CRUD (list/create/soft-delete) and wired optional evidence sources into base BA specs and substance recommendations (both on the substance detail page and on `/setup`). Substance detail tables now display attached evidence. Nav and command palette include a link to evidence sources. Files: `web/src/app/(app)/evidence-sources/page.tsx`, `web/src/app/(app)/evidence-sources/actions.ts`, `web/src/app/(app)/evidence-sources/create-evidence-source-form.tsx`, `web/src/lib/repos/evidenceSourcesRepo.ts`, `web/src/app/(app)/layout.tsx`, `web/src/components/command-palette.tsx`, `web/src/app/(app)/substances/[substanceId]/page.tsx`, `web/src/app/(app)/substances/[substanceId]/actions.ts`, `web/src/app/(app)/substances/[substanceId]/base-ba-form.tsx`, `web/src/app/(app)/substances/[substanceId]/recommendations-form.tsx`, `web/src/app/(app)/setup/page.tsx`, `web/src/app/(app)/setup/actions.ts`, `web/src/app/(app)/setup/base-ba-spec-form.tsx`. plan[283-303] plan[304-343] plan[541-555]
...
- [x] (2026-02-07 12:38Z) Fresh-eyes hardening: manual "Start cycle now" now also returns a clear user-facing error on the unique "one active cycle" constraint race (instead of leaking the raw constraint name) and adds a defensive `newCycleId` null-guard to prevent future refactors from introducing an uninitialized-id redirect. File: `web/src/app/(app)/cycles/actions.ts`.
- [x] (2026-02-07 12:43Z) Fresh-eyes correctness: `/today` no longer prompts "New cycle?" immediately after a user manually started an active cycle (it now detects when the active cycle started after the last event and assigns the next event to that cycle without creating an extra one). File: `web/src/app/(app)/today/actions.ts`. plan[261-282] plan[462-540] plan[666-670]
- [x] (2026-02-07 12:45Z) Fresh-eyes hardening: manual "Start cycle now" now also maps the cycle-number uniqueness race to the same user-facing "active cycle exists" error (avoids leaking raw constraint names in either unique-index order). File: `web/src/app/(app)/cycles/actions.ts`.
- [x] (2026-02-07 12:51Z) Security hardening: `/auth/callback` now sanitizes the optional `next` query param to prevent open redirects (only same-origin return URLs are allowed). File: `web/src/app/auth/callback/route.ts`. plan[636-689] plan[690-698]
- [x] (2026-02-07 12:54Z) UI polish: updated the root Next.js metadata from create-next-app defaults to the actual app name/description and aligned formatting with the rest of the codebase. File: `web/src/app/layout.tsx`.
- [x] (2026-02-07 13:03Z) Fresh-eyes hardening: clarified the `/today` "suggest new cycle" branch (fixed an `else` indentation/brace confusion and added an internal guard asserting `lastEventTs` exists for `'suggest_new_cycle'`), added a max ZIP size check for `/api/import`, and made `/api/export` unauthorized responses explicitly `no-store`. Files: `web/src/app/(app)/today/actions.ts`, `web/src/app/api/import/route.ts`, `web/src/app/api/export/route.ts`. plan[261-282] plan[462-540] plan[583-595] plan[682-689]
...
- [x] (2026-02-07 13:56Z) Next.js correctness: updated dynamic-route page props to treat `params` (and `searchParams`) as async, matching Next.js 15+ request API behavior and preventing subtle runtime mismatches. Files: `web/src/app/(app)/cycles/[cycleInstanceId]/page.tsx`, `web/src/app/(app)/devices/[deviceId]/page.tsx`, `web/src/app/(app)/formulations/[formulationId]/page.tsx`, `web/src/app/(app)/substances/[substanceId]/page.tsx`. plan[17-29]
...
- [x] (2026-02-07 14:46Z) UX fix: bulk-add flows now clearly communicate partial/failed imports. Bulk-add server actions append an error count to the success message, and the bulk-add success message is now colored: red when nothing was created, amber when partially successful, green when fully successful. Files: `web/src/app/(app)/substances/actions.ts`, `web/src/app/(app)/substances/bulk-add-substances-form.tsx`, `web/src/app/(app)/routes/actions.ts`, `web/src/app/(app)/routes/bulk-add-routes-form.tsx`, `web/src/app/(app)/formulations/actions.ts`, `web/src/app/(app)/formulations/bulk-add-formulations-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461] plan[644-659]
- [x] (2026-02-07 14:55Z) UX hardening: `createVialAction` now maps the "one active vial per formulation" unique-index race (`vials_one_active_per_formulation_key`) to a clear user-facing message (instead of leaking the raw constraint name). File: `web/src/app/(app)/inventory/actions.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[500-518] plan[660-676]
- [x] (2026-02-07 20:51Z) Performance: removed a formulations enrichment query waterfall by fetching related substances/routes/devices in parallel within `listFormulationsEnriched`. File: `web/src/lib/repos/formulationsRepo.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[699-713]
- [x] (2026-02-07 20:56Z) UX fix: reference-data create and bulk-add forms now call `router.refresh()` on success so server-rendered lists update immediately after saving (instead of requiring a manual refresh). Files: `web/src/app/(app)/substances/create-substance-form.tsx`, `web/src/app/(app)/substances/bulk-add-substances-form.tsx`, `web/src/app/(app)/routes/create-route-form.tsx`, `web/src/app/(app)/routes/bulk-add-routes-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461]
- [x] (2026-02-07 20:58Z) UX fix: formulations and devices create/bulk-add forms now call `router.refresh()` on success so same-page lists update immediately after saving. Files: `web/src/app/(app)/formulations/create-formulation-form.tsx`, `web/src/app/(app)/formulations/bulk-add-formulations-form.tsx`, `web/src/app/(app)/devices/create-device-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461] plan[653-659]
- [x] (2026-02-07 20:59Z) UX fix: distributions and evidence-sources create forms now call `router.refresh()` on success so same-page lists update immediately after saving. Files: `web/src/app/(app)/distributions/create-distribution-form.tsx`, `web/src/app/(app)/evidence-sources/create-evidence-source-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[283-303] plan[304-343] plan[422-461]
- [x] (2026-02-07 21:01Z) UX fix: inventory and orders create flows now call `router.refresh()` on success so same-page tables and dropdowns update immediately after saving (vials/vendors/orders/order items/vial generation). Files: `web/src/app/(app)/inventory/create-vial-form.tsx`, `web/src/app/(app)/orders/create-vendor-form.tsx`, `web/src/app/(app)/orders/create-order-form.tsx`, `web/src/app/(app)/orders/create-order-item-form.tsx`, `web/src/app/(app)/orders/generate-vials-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[500-518] plan[660-676]
- [x] (2026-02-07 21:03Z) UX fix: `/setup` quick-add forms now call `router.refresh()` on success so the model-coverage table and related selects reflect newly saved specs/calibrations immediately. Files: `web/src/app/(app)/setup/base-ba-spec-form.tsx`, `web/src/app/(app)/setup/device-calibration-form.tsx`, `web/src/app/(app)/setup/formulation-modifier-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461]
- [x] (2026-02-07 21:05Z) UX fix: substance and device detail edit forms now call `router.refresh()` on success so detail-page tables update immediately after saving (BA specs, recommendations, cycle rules, device calibrations). Files: `web/src/app/(app)/substances/[substanceId]/base-ba-form.tsx`, `web/src/app/(app)/substances/[substanceId]/recommendations-form.tsx`, `web/src/app/(app)/substances/[substanceId]/cycle-rule-form.tsx`, `web/src/app/(app)/devices/[deviceId]/create-calibration-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[283-303] plan[304-343] plan[644-652]
- [x] (2026-02-07 21:06Z) UX fix: formulation detail edit forms now call `router.refresh()` on success so newly added components/specs show up immediately without a manual refresh. Files: `web/src/app/(app)/formulations/[formulationId]/create-component-form.tsx`, `web/src/app/(app)/formulations/[formulationId]/component-modifier-spec-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[653-659]
- [x] (2026-02-07 21:09Z) Performance: removed a small `/orders` server-fetch waterfall by fetching order-item vial counts and vendor rows in parallel. File: `web/src/app/(app)/orders/page.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[500-518] plan[660-676]
- [x] (2026-02-07 21:21Z) UX fix: `/settings` profile-defaults form now calls `router.refresh()` on success so the page reflects the saved values immediately (and downstream pages pick up the updated timezone/units defaults without a manual refresh). File: `web/src/app/(app)/settings/settings-form.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[118-129] plan[583-595]
- [x] (2026-02-07 21:22Z) UX fix: data portability actions now refresh the app after destructive mutations so imported/deleted data is visible immediately without a manual reload. File: `web/src/app/(app)/settings/data-portability.tsx`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[583-595] plan[682-689]
- [x] (2026-02-07 21:26Z) UX hardening: mapped common DB constraint/unique-index errors to friendly user-facing messages for reference-data creation (instead of leaking raw index names). Added helper `toUserFacingDbErrorMessage(...)` and used it in substances/routes/devices/formulations actions (including bulk-add per-line errors). Files: `web/src/lib/errors/userFacingDbError.ts`, `web/src/app/(app)/substances/actions.ts`, `web/src/app/(app)/routes/actions.ts`, `web/src/app/(app)/devices/actions.ts`, `web/src/app/(app)/formulations/actions.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461] plan[644-659]
- [x] (2026-02-07 21:29Z) UX hardening: extended `toUserFacingDbErrorMessage(...)` usage to additional mutation actions (setup quick-add, detail-page spec/calibration forms, vendors/orders, evidence sources) so unique/constraint errors remain user-friendly across the app. Files: `web/src/app/(app)/setup/actions.ts`, `web/src/app/(app)/substances/[substanceId]/actions.ts`, `web/src/app/(app)/devices/[deviceId]/actions.ts`, `web/src/app/(app)/formulations/[formulationId]/actions.ts`, `web/src/app/(app)/orders/actions.ts`, `web/src/app/(app)/evidence-sources/actions.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[422-461] plan[500-518] plan[644-659]
- [x] (2026-02-07 21:34Z) Performance: removed a small server-side fetch waterfall on `/setup` and `/substances/[substanceId]` by fetching the profile (`getMyProfile() ?? ensureMyProfile()`) in parallel with other independent queries (folded into the existing `Promise.all(...)`). Files: `web/src/app/(app)/setup/page.tsx`, `web/src/app/(app)/substances/[substanceId]/page.tsx`. plan[422-461] plan[644-652]
- [x] (2026-02-07 21:42Z) UX hardening: order item creation and vial generation now map DB constraint/trigger errors via `toUserFacingDbErrorMessage(...)` (avoid leaking raw constraint names). File: `web/src/app/(app)/orders/actions.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[500-518] plan[660-676]
- [x] (2026-02-07 21:46Z) UX hardening: cycle rule save action now maps DB constraint errors via `toUserFacingDbErrorMessage(...)` (avoid leaking raw constraint names). File: `web/src/app/(app)/substances/[substanceId]/actions.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[261-282] plan[644-652]
- [x] (2026-02-07 21:50Z) UX hardening: distribution create action now maps DB constraint errors via `toUserFacingDbErrorMessage(...)` (avoid leaking raw constraint names), and `toUserFacingDbErrorMessage(...)` now includes user-friendly messages for common `distributions` check constraints. Files: `web/src/app/(app)/distributions/actions.ts`, `web/src/lib/errors/userFacingDbError.ts`. Also ran web quality gates: `npm run typecheck && npm test && npm run lint && npm run build`. plan[304-343] plan[422-461]
- [x] (2026-02-07 22:51Z) Docs: added a root `README.md` describing Peptaide's purpose/scope, local dev setup (Supabase + Next.js), and usage; updated `web/README.md` to point to the root README and include only quick commands (removes create-next-app boilerplate that didn't match this repo). Files: `README.md`, `web/README.md`. plan[1-29] plan[422-461] plan[690-698]
...
- [x] (2026-02-07 12:15Z) Cycle rules UX: added per-substance cycle rule overrides (gap-days threshold + auto-start-first-cycle) on the substance detail page, backed by upsert-style repo helpers and a soft-delete "remove override" action. Files: `web/src/lib/repos/cyclesRepo.ts`, `web/src/app/(app)/substances/[substanceId]/actions.ts`, `web/src/app/(app)/substances/[substanceId]/cycle-rule-form.tsx`, `web/src/app/(app)/substances/[substanceId]/page.tsx`. plan[261-282] plan[462-540]
...
- [ ] UI: implement `/today` as the primary keyboard-first logging surface (virtualized table, fast formulation selection, input_text parsing, enter-to-save, immediate next-row focus, non-blocking warnings for missing model pieces). (completed: `/today` end-to-end logging works and is now presented in the Stitch "Log & Inventory Hub" layout (Quick Log, multi-row grid, model coverage, today list, analytics widgets, and Control Center); it shows a day-scoped event list via `public.v_events_today`; command palette provides per-formulation log shortcuts; events support soft delete + restore; `/today` has a minimal multi-row log grid with Enter-to-save, ArrowUp/ArrowDown navigation, Shift+Enter reverse-save, and unbounded multi-line paste; remaining: richer spreadsheet navigation (Left/Right between formulation and dose cells, Tab/Shift+Tab, better bulk paste semantics), virtualization for larger days, and higher-fidelity formulation search/autocomplete in-cell). plan[9-13] plan[462-499] plan[638-643]
- [ ] UI: implement the cycle automation UX in `/today` (gap-based new-cycle suggestion with default-yes behavior that preserves fast entry) and complete the `/cycles` experience (timeline, manual create/end flows, and any remaining correction tools). (completed: `/today` server action assigns cycles automatically and prompts for "New cycle?" on gap thresholds with default-yes behavior; `/cycles` has list + detail pages plus "split cycle here" correction, manual "Start cycle now", and end/abandon actions; remaining: richer end-cycle UX (choose end time, break clock), manual cycle creation with custom timestamps, and any merge tooling). plan[261-282] plan[462-540] plan[666-670]

- [ ] UI: implement `/recommendations` surfaces within substance detail (manual entry now; clearly labeled as user-entered reference ranges, not advice) and comparisons shown in cycle detail. (completed: substance detail supports creating/listing/soft-deleting user-entered recommendations; evidence sources are now supported via `/evidence-sources` and can be attached to recommendations/specs; cycle detail surfaces recommended cycle/break ranges; remaining: dosing/frequency recommendations, and richer comparisons (cycle length vs recommended, total dose vs recommended, by route where possible)). plan[283-303] plan[541-555]
...
- [ ] UI: implement `/settings` including profile defaults (units, default MC N, cycle defaults) and data import/export with dry-run validation and dedupe (completed: profile defaults editor for timezone/units/MC N/cycle gap; added a signed-in CSV export bundle endpoint at `/api/export` and linked it from `/settings`; added CSV bundle import v1 with dry-run + apply (replace-only) plus a danger-zone "delete my data"; `/settings` is now presented in the Stitch "Master Data & Config Editor" workspace layout (left nav + substances grid + contextual right panel), while the original profile/data portability UI remains available under `?tab=app`; remaining: merge/dedupe semantics and id mapping for non-empty imports, clipboard/paste "template" imports for setup tables, and compatibility handling for schema evolution). plan[583-595] plan[682-689]
...
- [x] (2026-02-08 22:19Z) Import safety: fixed a non-replace simple events import rollback footgun by refusing to import into any non-empty dataset (except `profiles`). This prevents accidental data loss if an apply fails mid-way and triggers rollback. Updated docs and `/settings` UI messaging. Files: `web/src/lib/import/simpleEvents.ts`, `docs/SIMPLE_EVENTS_CSV_IMPORT.md`, `web/src/app/(app)/settings/data-portability.tsx`.
- [x] (2026-02-08 22:19Z) Testing harness hardening: made the conclusive browser harness more resilient to transient infra issues (retry `net::ERR_NETWORK_CHANGED` navigations, and longer retry loop for Supabase `db reset` 502 flakes). Reran and confirmed PASS on both direct port and Tailscale Serve origins. Evidence: PASS artifacts at `/tmp/peptaide-e2e-2026-02-08T22-04-03-316Z` (direct port) and `/tmp/peptaide-e2e-2026-02-08T22-13-14-975Z` (Tailscale Serve). File: `web/scripts/tbrowser/peptaide-e2e.mjs`.
- [x] (2026-02-08 22:50Z) Import correctness: hardened the simple events CSV parser to accept comma-grouped thousands in numeric cells (quoted, e.g. `"1,000"`) and to reject negative doses as per-row errors (prevents apply-time DB constraint failures and misleading “could not parse dose” warnings). Added regression tests. Files: `web/src/lib/import/simpleEvents.ts`, `web/src/lib/import/simpleEvents.test.ts`.
...
- [x] (2026-02-09 19:05Z) UI (design baseline): added the Google Stitch mockups and notes under `mockups/` (`mockups/logging_&_inventory_control_hub/` and `mockups/master_data_&_config_editor/`) so the exact HTML/CSS structure is preserved as a reference artifact during implementation.
- [x] (2026-02-09 19:05Z) UI (theme + shell): ported the Stitch dark theme tokens into Tailwind v4 CSS (`web/src/app/globals.css`), enabled Tailwind dark-mode class switching (`web/tailwind.config.ts`), set the app to default dark mode + Manrope font (`web/src/app/layout.tsx`), and refactored the authed app shell/header to match the "Pro Workspace" mockup style (`web/src/app/(app)/layout.tsx`, `web/src/components/command-palette.tsx`).
- [x] (2026-02-09 19:05Z) UI (Today hub): converted `/today` into the Stitch "Log & Inventory Hub" layout (left: Quick Log + existing multi-row grid + model coverage + today list; bottom: scrollable analytics widgets; right: Control Center inventory cards) and wired it to existing repos/views (today events, model coverage, inventory status, spend rollups, daily totals, cycle summary). Added an inline log-grid dosing hint sourced from user-entered dosing recommendations. Files: `web/src/app/(app)/today/page.tsx`, `web/src/app/(app)/today/today-log-grid.tsx`, `web/src/lib/repos/cycleSummaryRepo.ts`, `web/src/lib/repos/substanceRecommendationsRepo.ts`.
- [x] (2026-02-09 19:05Z) UI (Settings workspace): converted `/settings` into the Stitch "Master Data & Config Editor" workspace (left navigation + substances "master grid" + contextual right panel), while keeping the existing profile defaults + data portability surfaces under `?tab=app`. The right panel reuses the existing substance detail forms (base BA specs, recommendations, cycle rule override). Files: `web/src/app/(app)/settings/page.tsx`, `web/src/app/(app)/settings/settings-form.tsx`, `web/src/app/(app)/settings/data-portability.tsx`, `web/src/app/(app)/substances/[substanceId]/base-ba-form.tsx`, `web/src/app/(app)/substances/[substanceId]/recommendations-form.tsx`, `web/src/app/(app)/substances/[substanceId]/cycle-rule-form.tsx`, `web/src/lib/repos/cyclesRepo.ts`.
- [x] (2026-02-09 19:05Z) Testing: updated the conclusive browser harness for the new `/settings` tab layout and ensured the authed shell renders a `<main>` element so the sweep "page loaded" wait remains stable. Confirmed web quality gates (`npm run typecheck && npm test && npm run lint && npm run build`) and conclusive browser verification PASS. Evidence: PASS artifacts at `/tmp/peptaide-e2e-2026-02-09T19-00-47-840Z`. Files: `web/scripts/tbrowser/peptaide-e2e.mjs`, `web/src/app/(app)/layout.tsx`.
- [x] (2026-02-09 20:24Z) Testing: extended the conclusive browser harness with deep interaction coverage for the new Stitch-style `/settings` substances workspace. The harness now opens the substances master grid, navigates into the right-panel editor, exercises all three editor forms (base BA spec save, recommendation save, cycle rule save + remove), validates a minimal Stitch visual contract (dark mode + theme tokens + layout widths + primary button color), closes the editor, and sanity-checks cross-page wiring by asserting `/today` shows a `Rec:` hint after saving a dosing recommendation. Added stable `data-e2e` hooks for the workspace + forms. Evidence: `npm run e2e:browser` PASS artifacts at `/tmp/peptaide-e2e-2026-02-09T20-12-21-126Z`. Files: `web/scripts/tbrowser/peptaide-e2e.mjs`, `web/src/app/(app)/settings/page.tsx`, `web/src/app/(app)/substances/[substanceId]/base-ba-form.tsx`, `web/src/app/(app)/substances/[substanceId]/recommendations-form.tsx`, `web/src/app/(app)/substances/[substanceId]/cycle-rule-form.tsx`.
...
- Observation: ESLint `react-hooks/purity` flags `Date.now()` (and similar non-idempotent calls) inside React render, including in Next.js Server Components. The Stitch Control Center "Day X of Y" pill originally computed `dayOfCycle` from `Date.now()` and `cycle.start_ts`, which broke `npm run lint`.
  Evidence:
    Before: `npm run lint` failed with `Cannot call impure function during render` at `web/src/app/(app)/today/page.tsx` for `Date.now()`.
    Fix: derive `dayOfCycle` from `public.v_cycle_summary.cycle_length_days` (computed server-side with `now()` in SQL) so the React render remains pure, and `npm run lint` succeeds.
...
- Observation: The conclusive browser harness sweep step waits for a `<main>` element as a stable "page loaded" signal. When refactoring the authed app shell to match the Stitch mockups, the shell temporarily rendered children without a `<main>`, causing `npm run e2e:browser` to hang during `sweepPages(...)`.
  Evidence:
    Fix: always wrap authed page content in a semantic `<main>` in `web/src/app/(app)/layout.tsx`.
    After fix: `npm run e2e:browser` PASS artifacts at `/tmp/peptaide-e2e-2026-02-09T19-00-47-840Z`.
...
- Decision: Evidence sources are managed via a dedicated `/evidence-sources` page and are attached to bioavailability specs and recommendations via optional `evidence_source_id` dropdowns.
  Rationale: Keeps the domain model aligned with the schema (`evidence_sources` as a first-class table) without forcing citations into free-text notes, and avoids complicating the core setup/logging flow. Soft deletes hide sources from dropdowns while preserving historical references by id.
  Date/Author: 2026-02-07 / Codex
...
- Decision: Treat the Google Stitch mockups under `mockups/` as the UI design baseline and port their HTML/Tailwind structure directly into the Next.js App Router pages, then adapt wiring/behavior to the existing data model.
  Rationale: The app was functional but visually inconsistent; reusing the Stitch markup reduces "Frankenstein" design drift and keeps the UI coherent while preserving the already-working data flows.
  Date/Author: 2026-02-09 / Codex

- Decision: Map Stitch mockups onto existing MVP routes instead of introducing new top-level routes: "Log & Inventory Hub" becomes `/today`, and "Master Data & Config Editor" becomes `/settings` (defaulting to the substances workspace, with the original app settings/data portability UI moved under `?tab=app`).
  Rationale: `/today` and `/settings` are already the functional entry points for fast logging and configuration; keeping routes stable minimizes churn and keeps the conclusive browser harness deterministic.
  Date/Author: 2026-02-09 / Codex

- Decision: Match the mockup visual system by defaulting the app to dark mode (`<html class=\"dark\">`), using Tailwind v4 CSS theme tokens via `@theme inline` in `web/src/app/globals.css`, and loading the same display font (Manrope) and icon fonts (Material Icons) globally in `web/src/app/layout.tsx`.
  Rationale: Tailwind v4 is CSS-first; expressing tokens in `globals.css` keeps the theme close to the mockup source while allowing components/pages to remain mostly "copy/paste then wire up" without bespoke CSS.
  Date/Author: 2026-02-09 / Codex

- Decision: Where the mockups depict future-only capabilities (for example, vial scanning, a universal inline-edit grid for every reference table, or pharmacokinetics/half-life fields not present in the schema), keep the UI as a non-functional placeholder and document the gap, rather than inventing new schema/logic outside the MVP plan.
  Rationale: Preserves the mockup's layout and intent without expanding scope or creating unplanned data-model drift; missing capabilities remain visible and can be implemented deliberately in a later milestone.
  Date/Author: 2026-02-09 / Codex

## Outcomes & Retrospective

2026-02-09: UI refresh: ported the Google Stitch mockups into the live Next.js app while reusing the existing functional flows. The app now defaults to the Stitch visual system (dark mode + Manrope + Material Icons + Tailwind theme tokens). `/today` is presented as the Stitch "Log & Inventory Hub" (Quick Log, grid, model coverage, today list, analytics widgets, and inventory Control Center). `/settings` is presented as the Stitch "Master Data & Config Editor" (left nav + substances master grid + contextual right panel), with the original profile defaults + data portability tools retained under `?tab=app`. Conclusive browser harness PASS: `/tmp/peptaide-e2e-2026-02-09T19-00-47-840Z`.
...
6. `/today` is the Stitch-inspired "Log & Inventory Hub" that supports fast day-scoped logging (Quick Log + multi-row grid), non-blocking model coverage warnings, today event review (delete/restore), at-a-glance analytics widgets (daily administered totals, spend burn-rate), and an inventory Control Center (active vials + runway + cycle progress), all wired to the existing views/repos (`v_events_today`, `v_model_coverage`, `v_inventory_status`, `v_daily_totals_admin`, `v_spend_daily_weekly_monthly`, `v_cycle_summary`).
...
9. `/settings` is now the Stitch "Master Data & Config Editor" workspace (left nav + substances master grid + contextual right panel for BA specs/recommendations/cycle rules), with the original app settings and data portability tools (profile defaults, export/import, simple events import, delete-my-data) retained under `?tab=app`. `/analytics` remains a minimal read-only dashboard backed by daily totals + spend views.

What remains (next highest-leverage work):

1. UI consistency: restyle `/sign-in` to match the Stitch dark theme (or explicitly keep it as a light "public" surface), and confirm both magic-link and OTP-code flows remain smooth. Note: the conclusive browser harness already validates both auth paths functionally.
...
5. UI depth: refine `/setup` into a true wizard flow with completion state, implement the universal inline-edit "Master Grid" pattern for reference data (as implied by the Stitch editor mockup), and continue `/today` keyboard-first upgrades (cell navigation, virtualization, richer formulation search).
...
6. UI design artifacts from Google Stitch live under `mockups/` (HTML + screenshots). The app’s `/today` and `/settings` pages have been ported to reuse the Stitch markup and visual system (dark mode tokens, Manrope typography, and Material Icons), while staying wired to the existing Supabase-backed repos/actions.
...
2. Port the Google Stitch mockups for `/today` and `/settings` early (reuse the HTML/Tailwind structure and theme tokens) so follow-on UI work stays visually coherent and avoids "Frankenstein" styling drift.
3. Implement Setup Wizard before Today Log to avoid manual DB seeding.
4. Implement Today Log end-to-end (parse -> compute -> simulate -> store) before building complex analytics.
5. Implement inventory/orders and cycles/recommendations next.
6. Implement analytics and import/export last, but add the necessary views early to keep queries cheap.
...
2026-02-07: Improved bulk-add UX clarity by (1) including error counts in the success message and (2) coloring the message based on whether the operation was fully successful, partially successful, or fully failed. Updated `Progress` and re-ran web quality gates.

2026-02-07: Mapped the `vials_one_active_per_formulation_key` constraint race to a friendly message in `createVialAction` to avoid leaking raw constraint names in the inventory UI.

2026-02-07: Performance: removed an internal query waterfall in `listFormulationsEnriched` by fetching related substances/routes/devices in parallel (instead of sequential awaits). This reduces server render latency on pages that need enriched formulation options.

2026-02-07: UX fix: reference-data create/bulk-add forms that use `useActionState` now call `router.refresh()` on success so same-page server-rendered lists update immediately (substances + routes so far).

2026-02-07: UX fix: extended the `router.refresh()` pattern to formulations and devices reference pages so create/bulk-add operations update their list views immediately.

2026-02-07: UX fix: extended the `router.refresh()` pattern to `/distributions` and `/evidence-sources` so creating new distributions/evidence updates the list view immediately.

2026-02-07: UX fix: extended the `router.refresh()` pattern to inventory and orders forms (vial create, vendor/order/order-item create, and vial generation) so same-page tables update immediately after saves.

2026-02-07: UX fix: extended the `router.refresh()` pattern to the `/setup` quick-add forms so the model-coverage table and dropdowns update immediately after saving specs/calibrations.

2026-02-07: UX fix: extended the `router.refresh()` pattern to substance and device detail edit forms so newly saved specs/recommendations/calibrations show up immediately without a manual refresh.

2026-02-07: UX fix: extended the `router.refresh()` pattern to the formulation detail edit forms so new components and component modifier specs appear immediately after save.

2026-02-07: Performance: removed a small server-fetch waterfall on `/orders` by fetching order-item vial counts and vendor rows in parallel.

2026-02-07: UX fix: added `router.refresh()` after saving profile defaults on `/settings` so the page (and downstream timezone-sensitive pages) reflect the updated profile immediately.

2026-02-07: UX fix: `/settings` data portability actions now refresh the app after `apply` imports and after delete-my-data so other pages reflect the mutation immediately.

2026-02-07: UX hardening: added `toUserFacingDbErrorMessage(...)` to map common Postgres unique-index/constraint errors to friendly messages (avoid leaking raw index names) and applied it across reference-data create/bulk-add actions.

2026-02-07: UX hardening: extended `toUserFacingDbErrorMessage(...)` usage to setup/detail mutation actions and orders/evidence flows so constraint errors remain user-friendly throughout the app.

2026-02-07: Performance: removed a small server-side fetch waterfall on `/setup` and `/substances/[substanceId]` by fetching the profile (`getMyProfile() ?? ensureMyProfile()`) in parallel with other independent queries (folded into the existing `Promise.all(...)`), reducing server-render latency on those pages.

2026-02-07: UX hardening: ensured order item creation and vial generation actions map DB constraint/trigger errors via `toUserFacingDbErrorMessage(...)` (fixing a missed mapping in `web/src/app/(app)/orders/actions.ts`).

2026-02-07: UX hardening: cycle rule override save action now maps DB constraint errors via `toUserFacingDbErrorMessage(...)` (fixing a missed mapping in `web/src/app/(app)/substances/[substanceId]/actions.ts`).
