...
- [ ] UI: implement Setup Wizard A1-A6 (preferences, bulk add substances, bulk add routes, bulk add formulations, bulk add vials and/or generate from orders, base BA + modifiers entry) (completed: `/setup` exists as an onboarding hub and now embeds profile defaults, bulk-add primitives for substances/routes/formulations, a minimal "create vial" form, "generate vials from order items", a model-coverage gaps table, quick base BA spec entry, quick device calibration entry, and a minimal formulation modifier spec form; remaining: add a true bulk-vials paste/import (optional), add component-level modifier-spec entry (optional, but needed to override per-component behavior per compartment), and refine `/setup` into a true wizard flow with completion state). plan[422-461]
...
- [ ] UI: implement `/formulations` list + bulk add + detail page (components + modifier distributions, default device/vial settings) (completed: basic `/formulations` list + create; added a minimal formulation detail page to manage components with optional multiplier distributions; added a basic bulk-add textarea form; remaining: bulk grid/paste UX, per-compartment modifier specs editing, and default device/vial settings). plan[653-659]
