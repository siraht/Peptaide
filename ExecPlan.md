...
- [ ] UI: implement `/orders` and `/inventory` flows including order entry, order items, cost allocation preview, generate vials, set active vial, close/discard vial, runway estimates (completed: minimal `/inventory` page for creating/listing vials with runway estimates; remaining: `/orders` UI + order items + "generate vials", plus richer vial lifecycle actions and cost allocation preview). plan[500-518] plan[660-676]
