...
- [ ] UI: implement Setup Wizard A1-A6 (preferences, bulk add substances, bulk add routes, bulk add formulations, bulk add vials and/or generate from orders, base BA + modifiers entry) (completed: `/setup` exists as an onboarding hub and now embeds profile defaults, bulk-add primitives for substances/routes/formulations, a minimal "create vial" form, and a model-coverage gaps table; remaining: embed generate-from-orders for vials (and/or a true bulk-vials paste/import), embed base BA + modifier entry (not just links), then refine `/setup` into a true wizard flow with completion state). plan[422-461]
