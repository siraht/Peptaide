-- Make analytics views resilient when a `profiles` row is missing by falling back to UTC.
--
-- Rationale:
-- - The app attempts to ensure a `profiles` row exists, but treating it as a hard join makes views
--   brittle (and makes local RLS probes harder because simulated auth IDs are not present in auth.users).
-- - When profiles are missing, these views still produce correct results in UTC.
...
select
  e.user_id,
  (e.ts at time zone coalesce(p.timezone, 'UTC'))::date as day_local,
  f.substance_id,
...
with base as (
  select
    e.user_id,
    (e.ts at time zone coalesce(p.timezone, 'UTC')) as ts_local,
    e.cost_usd
  from public.administration_events e
  left join public.profiles p
    on p.user_id = e.user_id
...
daily_formulation as (
  select
    ...
    e.formulation_id,
    (e.ts at time zone coalesce(p.timezone, 'UTC'))::date as day_local,
    (now() at time zone coalesce(p.timezone, 'UTC'))::date as today_local,
    sum(e.dose_mass_mg) as administered_mg
  from public.administration_events e
  left join public.profiles p
    on p.user_id = e.user_id
