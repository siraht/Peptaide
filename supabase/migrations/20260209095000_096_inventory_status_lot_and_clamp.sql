-- Inventory UX hardening:
-- - include vials.lot in v_inventory_status so the UI can show vial numbers (e.g. vial_1..vial_10)
-- - clamp remaining mass/volume and runway estimates to non-negative values (avoid confusing negatives when
--   imported spreadsheet doses slightly exceed nominal vial content due to rounding/truncation)
...
select
  ...
  case
    when vc.total_volume_ml is null then null
    else greatest(0, vc.total_volume_ml - coalesce(u.used_volume_ml, 0))
  end as remaining_volume_ml,
  vc.cost_usd,
  ...
  case
    ...
    else greatest(0, vc.content_mass_mg - coalesce(u.used_mass_mg, 0)) / au.avg_daily_administered_mg_14d
